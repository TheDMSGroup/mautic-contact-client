...
if (empty($preview)):
    ...
    <script>
        var MauticContactClientHandler = function (messageType, message) {
            // Store the HTML
        ...
        MauticFormCallback["<?php echo $jsFormName; ?>"] = {
            onMessageSet: function (data) {
                if (data.message) {
                    MauticContactClientHandler(data.type);
                }
            },
            onErrorMark: function (data) {
                if (data.validationMessage) {
                    MauticContactClientHandler('error', data.validationMessage);
...
<?php
$formExtra = <<<EXTRA
<input type="hidden" name="mauticform[contactclientId]" id="mauticform<?php echo $formName ?>_contactclient_id" value="$contactclientId"/>
EXTRA;
