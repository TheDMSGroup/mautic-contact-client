...
$color   = \MauticPlugin\MauticContactClientBundle\Model\ContactClientModel::isLightColor($props['colors']['primary']) ? '000000' : 'ffffff';
$animate = (!empty($preview) && !empty($props['animate'])) ? ' mf-animate' : '';
?>
<div class="mautic-contactclient mf-bar mf-bar-<?php echo $props['bar']['size']; ?> mf-bar-<?php echo $props['bar']['placement']; ?><?php if ($props['bar']['sticky']) {
    echo ' mf-bar-sticky';
} ?><?php echo $animate; ?>" style="background-color: #<?php echo $props['colors']['primary']; ?>;">

    <div class="mf-content">
        <?php if (in_array($htmlMode, ['editor', 'html'])): ?>
            <?php echo html_entity_decode($contactclient[$htmlMode]); ?>
        <?php else: ?>
        <div class="mf-headline"><?php echo $props['content']['headline']; ?></div>
        <?php if ($contactclient['type'] == 'form'): ?>
            {contactclient_form}
        <?php elseif ($contactclient['type'] == 'link'): ?>
...
if ($props['bar']['allow_hide']): ?>
    <div class="mf-copy-to-parent mf-bar-collapser mf-bar-collapser-<?php echo $props['bar']['placement']; ?> mf-bar-collapser-<?php echo $props['bar']['size']; ?><?php if ($props['bar']['sticky']) {
                ...
                } ?> mf-bar-collapser-<?php echo $contactclient['id']; ?>" style="background-color: #<?php echo $props['colors']['primary']; ?>; color: #<?php echo $props['colors']['text']; ?>;">
        <style scoped>
        ...
        <a class="mf-bar-collapser-icon" href="javascript:void(0)"<?php if (!empty($preview)) {
            ...
            <svg style="overflow: hidden;" xmlns="http://www.w3.org/2000/svg" width="<?php echo $size; ?>" version="1.1"
                ...
                <g transform="scale(<?php echo $scale; ?>) rotate(<?php echo $direction; ?> <?php echo $transformSize; ?> <?php echo $transformSize; ?>)">
                    ...
                    <defs>
                        <?php $color = \MauticPlugin\MauticContactClientBundle\Model\ContactClientModel::isLightColor($props['colors']['primary']) ? '000000'
                            : 'ffffff'; ?>
...
<?php if ($props['bar']['push_page'] && $props['bar']['placement'] == 'top'): ?>
    <div class="mf-move-to-parent mf-bar-spacer mf-bar-spacer-<?php echo $props['bar']['size']; ?> mf-bar-spacer-<?php echo $contactclient['id']; ?>"></div>
<?php endif; ?>
