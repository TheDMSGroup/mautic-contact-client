...
/**
 * ContactEventLogRepository.
 */
class ContactEventLogRepository extends CommonRepository
{
    use TimelineTrait;
    ...
    public function getEntities(array $args = [])
    {
        ...
        if (empty($args['contactclient_id'])) {
            $q->join($alias.'.event', 'e')
                ->join($alias.'.contactclient', 'c');
        } else {
            $q->andWhere(
                $q->expr()->eq('IDENTITY('.$this->getTableAlias().'.contactclient)', (int)$args['contactclient_id'])
            );
        }
        ...
        $query = $this->getEntityManager()
            ...
            ->select(
                'll.id as log_id,
                    ...
                    c.name AS contactclient_name,
                    c.description AS contactclient_description,
                    ll.metadata,
                    e.type,
            ...
            ->from(MAUTIC_TABLE_PREFIX.'contactclient_contact_event_log', 'll')
            ->leftJoin('ll', MAUTIC_TABLE_PREFIX.'contactclient_events', 'e', 'll.event_id = e.id')
            ->leftJoin('ll', MAUTIC_TABLE_PREFIX.'contactclients', 'c', 'll.contactclient_id = c.id')
    ...
    public function getUpcomingEvents(array $options = null)
    {
        ...
        $query->from(MAUTIC_TABLE_PREFIX.'contactclient_contact_event_log', 'll')
            ->select(
                'll.event_id,
                    ll.contactclient_id,
                    ll.trigger_date,
            ...
            ->leftJoin('ll', MAUTIC_TABLE_PREFIX.'contactclient_events', 'e', 'e.id = ll.event_id')
            ->leftJoin('ll', MAUTIC_TABLE_PREFIX.'contactclients', 'c', 'c.id = e.contactclient_id')
     ...
     * @return array
     */
    public function getContactClientLogCounts($contactclientId, $excludeScheduled = false, $excludeNegative = true)
    {
        $q = $this->_em->getConnection()->createQueryBuilder()
            ->from(MAUTIC_TABLE_PREFIX.'contactclient_contact_event_log', 'o')
            ->innerJoin(
                'o',
                MAUTIC_TABLE_PREFIX.'contactclient_leads',
                'l',
                'l.contactclient_id = '.(int)$contactclientId.' and l.manually_removed = 0 and o.lead_id = l.lead_id and l.rotation = o.rotation'
            );

        $expr = $q->expr()->andX(
            $q->expr()->eq('o.contactclient_id', (int)$contactclientId)
        );
        ...
        $failedSq->select('null')
            ->from(MAUTIC_TABLE_PREFIX.'contactclient_contact_event_failed_log', 'fe')
            ->where(
     ...
     * @param $leadId
     */
    public function removeScheduledEvents($contactclientId, $leadId)
    {
        $conn = $this->_em->getConnection();
        $conn->delete(
            MAUTIC_TABLE_PREFIX.'contactclient_contact_event_log',
            [
                'lead_id' => (int)$leadId,
    ...
    public function updateLead($fromLeadId, $toLeadId)
    {
        ...
        $results = $this->_em->getConnection()->createQueryBuilder()
            ->select('cl.event_id')
            ->from(MAUTIC_TABLE_PREFIX.'contactclient_contact_event_log', 'cl')
        ...
        $q = $this->_em->getConnection()->createQueryBuilder();
        $q->update(MAUTIC_TABLE_PREFIX.'contactclient_contact_event_log')
        ...
        if (!empty($exists)) {
            ...
            $this->_em->getConnection()->createQueryBuilder()
                ->delete(MAUTIC_TABLE_PREFIX.'contactclient_contact_event_log')
                ->where('lead_id = '.(int)$fromLeadId)
                ->execute();
        } else {
            $q->execute();
    ...
    public function getChartQuery($options)
    {
        ...
        $query->select('ll.id, ll.date_triggered')
            ->from(MAUTIC_TABLE_PREFIX.'contactclient_contact_event_log', 'll')
            ->join('ll', MAUTIC_TABLE_PREFIX.'contactclient_events', 'e', 'e.id = ll.event_id');
